...
// import Observable = require("dstore/Observable")
...
view.when(() => {
	...
	// dataStore.add({"__OBJECTID": 0, "something": "Andy"})

	console.log('dataStore', dataStore)

	// let didDelete = dataStore.remove(0)

	// console.log('didDelete', didDelete)

	// console.log('dataStore before', dataStore)
			...
			map.ground.queryElevation(event.mapPoint, {
				...
				.then(function(elevationResult){
					...
					statePlaneLayer.queryFeatures(query).then(function(statePlaneResults) {
						...
						let thisFips = statePlaneResults.get('features')[0].get('attributes').FIPSZONE;
						let testFips = thisFips.length < 4 ? "0".concat(thisFips.toString()) : thisFips;
						// console.log('attributes', statePlaneResults.get('features')[0].get('attributes'))
						// console.log('testFips', testFips)
						let thisWkid = zones.find(z => z.fips.toString() === testFips.toString())
						// console.log('thisWkid', thisWkid)
						let thisSpatialReference = new SpatialReference(thisWkid)
						// console.log('thisSpatialReference', thisSpatialReference)
						let thisProjectedGeometry = Projection.project(elevationResult.geometry, thisSpatialReference)
						...
						let point = {
							...
							demr: elevationResult.sampleInfo[0].demResolution,
							dems: 'ESRI WorldElevation Service'
						};
						layer.source.add(point)
						dataStore.add(point)
						// console.log('point', point)
						// let newData = layer.source.map(l => l.get('attributes'));
						// console.log('newData', newData)
						// dataStore.objectStore.data = newData;
						// console.log('point', point);
						// console.log('dataStore', dataStore)
						dataStore.forEach((x,i) => console.log(i, x))
